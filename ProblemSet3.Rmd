...
The sensitivity matrix shows that changing probability of a juvenile becoming an adult will increase the rate of growth the most out of all the parameters. The next most sensitive parameter is the survival rate of eggs/laval. The 31.86 in row 3 and column 1 would change the rate of growth the most, however, there is no parameter for adults transitioning back into eggs. The elasticity matrix shows that the proportional sensitivity is highest in the adult survival rate, and the probability of a subadult remaining a subadult. 

## 4) Using the calculations in part (c), or additional calculations, address the question you proposed in part (b).

The stage that is most sensitive to intervention would be the juvnile stage because the ratios from the elasticity matrix and the values from the sensitivity matrix show that stage being the most receptive to intervention. If the population was growing too much for a population, then it would be useful to understand the survival rates of eggs/laval, and juveniles and how to decrease these areas to lessen overall growth. We were also under the assumption that (something we still have to finish this sentence)
