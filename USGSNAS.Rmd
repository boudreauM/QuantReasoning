...
output:
  ...
  pdf_document: default
---
The data source is the United States Geological Survey (USGS) Nonindigenous Aquatic 
Species (NAS) data set. It is a collection of zebra mussel sightings across the United States. 
Zebra mussels are an incredibly damaging invasive species in the continental US.  
...
```
These maps demonstrate the spread of zebra mussels in increments of one year.
...
lake_temp_counts <- read_excel("lake high temp and counts.xlsx")
...
```
The average temperature of each lake throughout the year shows that Lake Erie get sthe warmest and Lake Superior is the coldest. The number of sightings per lake per year was compared with the high temperature of each lake for the years 1995-2018. Since the temperature data begins with 1995, there could not be a comparison including sighting data for any year prior to 1995. There are no significant correlaitons between the high temperature of the lake and the number of sightings that occurred during any given year. Therefore, it can be concluded that temperature is likely not a driving factor of zebra mussel spread or sighting collection. 
```{r Bias, echo=FALSE}
...
```
There are more sightings during the summer months (approx. July-October). This is likely due to the fact that most sampling is done during the summer when the lakes are not frozen. Therefore, there is a significant bias for what time of year there are the most sightings of zerba mussels.
=======



This analysis was mostly a descriptive visualization--where, and when, are zebra mussels found? Additional linear models demonstrate no significant relationship of zebra mussel occurrences to the size (area or volume) of the lake, or the time period.
