...
#Want to use this chunk to see if there is a correlation between number of occurances in each lake and their size. Linear Regression Analysis. If there isn't a positive correlation then would it be more around tourist areas? or more populated with boating areas. 
 ...
 sup_counts <-table(Superior$HUC.8.Number)
 sup_counts <- sup_counts[2,]
 mich_counts <-table(Michigan$HUC.8.Number)
 huron_counts <-table(Huron$HUC.8.Number)
 erie_counts <-table(Erie$HUC.8.Number)
 ont_counts <-table(Ontario$HUC.8.Number)
 zm_occurances <-c(sup_counts, mich_counts, huron_counts, erie_counts, ont_counts)
 
 sizeDat <- data.frame(size_of_lake, zm_occurances)
 
 model <- lm(sziezm_occurances)
